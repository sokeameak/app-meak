...
if ( !class_exists('FormLayerUpdateChecker_3_2', false) ):
...
class FormLayerUpdateChecker_3_2{
	...
	public function __construct($metadataUrl, $pluginFile, $slug = '', $checkPeriod = 12, $optionName = '', $muPluginFile = ''){
		...
		$this->upgraderStatus = new FormLayer_PucUpgraderStatus_3_2();
	 ...
	 * @return FormLayer_PucScheduler_3_2
	 */
	protected function createScheduler($checkPeriod) {
		return new FormLayer_PucScheduler_3_2($this, $checkPeriod);
	}
	...
	public function requestInfo($queryArgs = array()){
		...
		if ( !is_wp_error($status) ){
			$pluginInfo = FormLayerInfo_3_2::fromJson($result['body']);
			if ( $pluginInfo !== null ) {
	 ...
	 *
	 * @uses FormLayerUpdateChecker::requestInfo()
	 *
	 * @return FormLayerUpdate_3_2 An instance of FormLayerUpdate, or NULL when no updates are available.
	 */
	public function requestUpdate(){
		...
		$update = FormLayerUpdate_3_2::fromFormLayerInfo($pluginInfo);
	 ...
	 *
	 * @return FormLayerUpdate_3_2|null
	 */
	...
	public function getUpdateState() {
		...
		if ( isset($state, $state->update) && is_object($state->update) ) {
			$state->update = FormLayerUpdate_3_2::fromObject($state->update);
		}
		return $state;
	}
	...
	private function setUpdateState($state) {
		if ( isset($state->update) && is_object($state->update) && method_exists($state->update, 'toStdClass') ) {
			$update = $state->update; /** @var FormLayerUpdate_3_2 $update */
	 ...
	 * @param FormLayerUpdate_3_2 $updateToAdd
	 * @return StdClass
	 */
	...
	public function addCheckForUpdatesLink($pluginMeta, $pluginFile) {
		...
		if ( $isRelevant && current_user_can('update_plugins') ) {
			...
			$linkText = apply_filters('puc_manual_check_link-' . $this->slug, __('Check for updates', 'formlayer-pro'));
			if ( !empty($linkText) ) {
	...
	public function displayManualCheckResult() {
		if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->slug) ) {
			...
			if ( $status == 'no_update' ) {
				$message = __('This plugin is up to date.', 'formlayer-pro');
			} else if ( $status == 'update_available' ) {
				$message = __('A new version of this plugin is available.', 'formlayer-pro');
			} else {
				// Translators: %s is the status of the update checker.
				$message = sprintf(__('Unknown update checker status "%s"', 'formlayer-pro'), htmlentities($status));
			}
			printf(
	 ...
	 * successfully, the first argument passed will be an instance of  FormLayerInfo. Otherwise, 
	 * it will be NULL. The second argument will be the corresponding return value of 
	 * wp_remote_get (see WP docs for details).
	 *  
	 * The callback function should return a new or modified instance of FormLayerInfo or NULL.
	 * 
	 * @uses add_filter() This method is a convenience wrapper for add_filter().
	 * 
	 * @param callable $callback
	...
	public function initDebugBarPanel() {
		...
		if ( class_exists('Debug_Bar', false) && file_exists($debugBarPlugin) ) {
			...
			$this->debugBarPlugin = new FormLayer_PucDebugBarPlugin_3_2($this);
		}
	}
...
if ( !class_exists('FormLayerInfo_3_2', false) ):
...
class FormLayerInfo_3_2 {
	...
	/**
	 * Create a new instance of FormLayerInfo from JSON-encoded plugin info 
	 * returned by an external update API.
	 * 
	 * @param string $json Valid JSON string representing plugin info.
	 * @return FormLayerInfo_3_2|null New instance of FormLayerInfo, or NULL on error.
	 */
...
if ( !class_exists('FormLayerUpdate_3_2', false) ):
...
class FormLayerUpdate_3_2 {
	...
	/**
	 * Create a new instance of FormLayerUpdate from its JSON-encoded representation.
	 * 
	 * @param string $json
	 * @return FormLayerUpdate_3_2|null
	 */
	public static function fromJson($json){
		...
		$pluginInfo = FormLayerInfo_3_2::fromJson($json);
		if ( $pluginInfo != null ) {
			return self::fromFormLayerInfo($pluginInfo);
		} else {
			return null;
		}
	}
	...
	/**
	 * Create a new instance of FormLayerUpdate based on an instance of FormLayerInfo.
	 * Basically, this just copies a subset of fields from one object to another.
	 * 
	 * @param FormLayerInfo_3_2 $info
	 * @return FormLayerUpdate_3_2
	 */
	public static function fromFormLayerInfo($info){
		return self::fromObject($info);
	}
	
	/**
	 * Create a new instance of FormLayerUpdate by copying the necessary fields from 
	 * another object.
	 *  
	 * @param StdClass|FormLayerInfo_3_2|FormLayerUpdate_3_2 $object The source object.
	 * @return FormLayerUpdate_3_2 The new copy.
	 */
	 ...
	 * a FormLayerUpdate. Useful for serialization and caching, as it avoids
...
if ( !class_exists('FormLayer_PucScheduler_3_2', false) ):

/**
 * The scheduler decides when and how often to check for updates.
 * It calls @see FormLayerUpdateChecker::checkForUpdates() to perform the actual checks.
 *
 * @version 3.2
 */
class FormLayer_PucScheduler_3_2 {
	public $checkPeriod = 12; //How often to check for updates (in hours).
	public $throttleRedundantChecks = false; //Check less often if we already know that an update is available.
	public $throttledCheckPeriod = 72;

	/**
	 * @var FormLayerUpdateChecker_3_2
	 */
	 ...
	 *
	 * @param FormLayerUpdateChecker_3_2 $updateChecker
...
if ( !class_exists('FormLayer_PucUpgraderStatus_3_2', false) ):
 ...
 */
#[\AllowDynamicProperties]
class FormLayer_PucUpgraderStatus_3_2 {
...
if ( !class_exists('FormLayer_PucFactory', false) ):

/**
 * A factory that builds instances of other classes from this library.
 *
 * When multiple versions of the same class have been loaded (e.g. FormLayerUpdateChecker 1.2
 * and 1.3), this factory will always use the latest available version. Register class
 * versions by calling {@link FormLayer_PucFactory::addVersion()}.
 *
 * At the moment it can only build instances of the FormLayerUpdateChecker class. Other classes
 * are intended mainly for internal use and refer directly to specific implementations. If you
 * want to instantiate one of them anyway, you can use {@link FormLayer_PucFactory::getLatestClassVersion()}
 * to get the class name and then create it with <code>new $class(...)</code>.
 */
#[\AllowDynamicProperties]
class FormLayer_PucFactory {
	protected static $classVersions = array();
	protected static $sorted = false;

	/**
	 * Create a new instance of FormLayerUpdateChecker.
	 *
	 * @see FormLayerUpdateChecker::__construct()
	 *
	 * @param $metadataUrl
	 * @param $pluginFile
	 * @param string $slug
	 ...
	 *
	 * @param string $generalClass Class name without version numbers, e.g. 'FormLayerUpdateChecker'.
	 * @param string $versionedClass Actual class name, e.g. 'FormLayerUpdateChecker_1_2'.
...
FormLayer_PucFactory::addVersion('FormLayerUpdateChecker', 'FormLayerUpdateChecker_3_2', '3.2');
FormLayer_PucFactory::addVersion('FormLayerUpdate', 'FormLayerUpdate_3_2', '3.2');
FormLayer_PucFactory::addVersion('FormLayerInfo', 'FormLayerInfo_3_2', '3.2');
FormLayer_PucFactory::addVersion('FormLayer_PucGitHubChecker', 'FormLayer_PucGitHubChecker_3_2', '3.2');
