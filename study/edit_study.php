...
// Fetch existing study record
$study = [];
$stmt = $conn->prepare("SELECT * FROM tb_study WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($id_stu) || empty($id_time) || empty($id_code) || empty($price) || empty($start_date) || empty($end_date)) {
        ...
        // Update database
        $sql = "UPDATE tb_study SET id_stu=?, id_time=?, id_code=?, price=?, start_date=?, end_date=? WHERE id=?";
        $stmt = $conn->prepare($sql);
        
        if ($stmt) {
            $stmt->bind_param("iisdssi", $id_stu, $id_time, $id_code, $price, $start_date, $end_date, $id);
            if ($stmt->execute()) {
                $success = "Study record updated successfully!";
                // Refresh data
                $study['id_stu'] = $id_stu;
                $study['id_time'] = $id_time;
                $study['id_code'] = $id_code;
                $study['price'] = $price;
                $study['start_date'] = $start_date;
                $study['end_date'] = $end_date;
            } else {
                $error = "Error updating study record: " . $stmt->error;
            }
            $stmt->close();
        } else {
            ...
            <div class="card">
                ...
                <form method="POST">
                    <div class="form-group">
                        ...
                        <select id="id_stu" name="id_stu" required>
                            <option value="">-- Select Student --</option>
                            <?php foreach ($students as $s) echo "<option value='{$s['ID']}' " . ($study['id_stu'] == $s['ID'] ? 'selected' : '') . ">{$s['student_name']}</option>"; ?>
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <select id="id_time" name="id_time" required>
                            ...
                            <?php foreach ($times as $t) echo "<option value='{$t['id']}' " . ($study['id_time'] == $t['id'] ? 'selected' : '') . ">{$t['time']}</option>"; ?>
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <select id="id_course" name="id_course" required>
                            ...
                            <?php foreach ($courses as $c) echo "<option value='{$c['ID']}' " . ($study['id_code'] == $c['ID'] ? 'selected' : '') . ">{$c['Course']}</option>"; ?>
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <select id="price" name="price" required>
                            ...
                            foreach ($prices as $p) echo "<option value='$p' " . ($study['price'] == $p ? 'selected' : '') . ">$p</option>";
                            ?>
                        </select>
                    </div>
                    <div class="form-group"><label for="start_date">Start Date:</label><input type="date" id="start_date" name="start_date" value="<?php echo htmlspecialchars($study['start_date']); ?>" required></div>
                    <div class="form-group"><label for="end_date">End Date:</label><input type="date" id="end_date" name="end_date" value="<?php echo htmlspecialchars($study['end_date']); ?>" required></div>

                    <div class="button-group">
                        <button type="submit" class="btn">Update Study Record</button>
