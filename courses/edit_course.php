...
// Fetch course record from database
$course = [
    'CourseID' => $CourseID,
    'Course' => '',
    'Note' => ''
...
if (empty($CourseID)) {
    ...
    $sql = "SELECT * FROM tb_course WHERE CourseID = ?";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($Course)) {
        ...
        // Update database
        $sql = "UPDATE tb_course SET Course = ?, Note = ? WHERE CourseID = ?";
        $stmt = $conn->prepare($sql);
        
        if ($stmt) {
            $stmt->bind_param("sss", $Course, $Note, $CourseID);
            if ($stmt->execute()) {
    ...
    <div class="container">
        ...
        <div class="main-content">
            ...
            <div class="card">
                ...
                <?php if (empty($error) || strpos($error, "Error updating") === 0): ?>
                    <form method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <input 
                                ...
                                value="<?php echo htmlspecialchars($course['Course'] ?? ''); ?>"
                                required
                            >
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            ><?php echo htmlspecialchars($course['Note'] ?? ''); ?></textarea>
                        </div>
