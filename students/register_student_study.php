...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Validate inputs
    if (empty($student_name) || empty($sex) || empty($dob) || empty($id_code) || empty($id_time) || empty($price) || empty($start_date) || empty($end_date)) {
        $error = "Please fill in all required fields (Student Name, Sex, DOB, Course, Time, Price, Dates)!";
    } else {
        // Start Transaction
        ...
        try {
            ...
            // Commit
            $conn->commit();
            log_siem_event($conn, $_SESSION['user'], 'REGISTER_STUDENT_STUDY', "Registered student: $student_name with course ID: $id_code");
            $success = "Student and Study record added successfully!";
...
<head>
    ...
    <title>Register Student & Study</title>
...
<body>
    <div class="container">
        <div class="sidebar">
            ...
            <a href="register_student_study.php" style="background: #34495e;"><i class="fa-solid fa-registered"></i> Register & Study</a>
